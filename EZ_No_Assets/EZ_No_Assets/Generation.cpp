...
#if 0
Generation::Generation(OLGModel &p_model, OLGModelMemFn p_fn, int state, int whichGen, VectorXd &p_Up1, VectorXd &p_Ep1, VectorXd &p_Wp1)
...
//	return CALL_MEMBER_FN(*m_model, m_fn)(m_state, m_whichGen, x, *Up1, *Ep1, *Wp1);
...
double Generation::operator()(const std::vector<double> &x, std::vector<double> &grad) {
	//	return CALL_MEMBER_FN(*m_model, m_fn)(m_state, m_whichGen, x[0], *Up1, *Ep1, *Wp1);
